    ...
    <div class="row">
        <div class="col-3">
            <ul class="list-group list-group-flush">
                <li class="list-group-item list-group-item-action" @click="chooseStudent({ id: '' })">
                    Chưa phân lớp
                </li>
                <li class="list-group-item list-group-item-action" v-for="lh in lophocs" :key="lh.id" @click="choose(lh)">
                    {{ lh.name }}
                </li>
            </ul>
        </div>
                ...
                <div class="col">
                    ...
                    <div v-else>
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <tr v-for="hocsinh in hocsinhs">
                                    <td>{{ hocsinh.name }}</td>
                                    <td class="text-right">{{ numberWithCommas(hocsinh.parent.debt) }}</td>
                                    <td>
                                        <a class="btn btn-info" @click="choose(hocsinh)"
                                            :href="`/hocsinh/${hocsinh.id}`">Xem chi tiết</a>
                                        <a class="btn btn-warning" @click="choose(hocsinh)"
                                            :href="`/hocsinh/congno/${hocsinh.id}`">Xem nợ</a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
